...
本项目管理面板是一个 Cloudflare Worker（`workers/`），部署方式二选一：
**Cloudflare 一键部署**（推荐，点按钮即可）或 **本地 wrangler 部署**（适合改代码调试）。
两种方式产物相同：`https://aishell-admin.<ACCOUNT_ID>.workers.dev` 上的管理面板。
...
## 方式 A：Cloudflare 一键部署（推荐）

### 前提

1. 仓库保持 **public**（Cloudflare 需要拉取代码）；
2. 有 Cloudflare 账号（免费即可）。
...
4. 点击 **Deploy**。Cloudflare 会用 `package.json` 里的 deploy 脚本
   （`node sync-kv.mjs && wrangler deploy`）构建部署：
   - 自动把 `admin.html`、代理源码、容器部署脚本上传到 KV；
   - 自动部署 Worker。
...
- Cloudflare 账号；
- 两种认证方式任选：
  - 交互式：`npx wrangler login`（浏览器授权）；
  - API Token：环境变量 `CLOUDFLARE_API_TOKEN`（权限：Workers Scripts Edit + KV Storage Edit）。

### 步骤

```bash
# 1. 进入 workers 目录并安装 wrangler
cd workers
npm i -D wrangler

# 2. 认证（二选一）
npx wrangler login                 # 方式一：浏览器登录
# export CLOUDFLARE_API_TOKEN=xxx  # 方式二：API Token

# 3. 创建 KV namespace（只需一次），把输出的 id 填入 wrangler.toml
npx wrangler kv namespace create KV
#   输出形如：id = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"  → 替换 wrangler.toml 里的
#   REPLACE_WITH_KV_NAMESPACE_ID

# 4. 设置 ADMIN_KEY（面板登录密钥，自定义强随机值，只填一次）
npx wrangler secret put ADMIN_KEY
#   按提示输入密钥值（不会显示、不会入库）

# 5. 一键部署（自动：上传 admin.html/代理源码/部署脚本到 KV → 部署 Worker）
npm run deploy
#   等价于：node sync-kv.mjs && npx wrangler deploy

# 6. 验证
curl -fsS https://aishell-admin.<ACCOUNT_ID>.workers.dev/api/health
#   返回 {"ok":true,...} 即部署成功
```
...
npm run sync    # 只把脚本重新上传到 KV（改了 admin.html / 代理源码 / deploy-remote.sh 后必跑）
npm run deploy  # 同步 + 部署
```
...
```bash
cd workers
npm run deploy          # 一键同步 + 重新部署
npx wrangler tail       # 实时查看 Worker 日志（排障用）
```
...
| 对比项 | A：一键部署 | B：本地部署 |
|---|---|---|
| 上手难度 | 最低（点按钮） | 需装 Node/wrangler |
| 适合场景 | 首次部署、快速体验 | 改代码、调试、多次迭代 |
| 创建 KV | 自动 | 手动（一次） |
| 设置 ADMIN_KEY | 部署时提示填写 | `wrangler secret put` |
| 传脚本到 KV | 自动（deploy 脚本） | 自动（npm run deploy） |
| 自定义资源名 | 配置页可改 | wrangler.toml |
